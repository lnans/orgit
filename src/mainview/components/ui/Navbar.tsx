...
export function Navbar({ children }: { children?: React.ReactNode }) {
	return (
		<div className="flex flex-col w-84 pt-7 bg-nav bg-opacity-20 border-r border-r-neutral-800">
			{children}
		</div>
	);
}
...
export function NavbarTitle({ label, children }: { label: string; children?: React.ReactNode }) {
	return (
		<div className="inline-flex items-center gap-0.5 h-6 px-2.5">
			<p className="text-xxs text-neutral-500">{label}</p>
...
export function NavbarItem({
	...
	return (
		<div className="group inline-flex items-center gap-2 h-5 px-1 ms-1.5 me-2">
			<IconLabel className="text-neutral-400" size={12} />
			<p className="text-xxs text-neutral-400">{label}</p>
			<div className="flex-1" />
...
export function NavbarSubItem({ label, children }: { label: string; children?: React.ReactNode }) {
	return (
		<div className="group inline-flex items-center mx-2.5 h-5 rounded-md text-neutral-400 hover:bg-neutral-500/10 hover:text-neutral-300 transition-colors duration-75 select-none">
...
export function NavbarItemEmpty({ label }: { label: string }) {
	return (
		<div className="inline-flex items-center h-5">
			<p className="ps-7.5 text-xxs text-neutral-500">{label}</p>
		</div>
	);
}
